...
import SpiralIntervals from './spiral_intervals';
...
export default class Spiral extends MusicModule {
	...
	public keyLocations: Map<number, MRE.Vector3> = new Map();
	...
	public setScale(scale: number) {
		this.SpiralScale = scale;
		if (this.keyboardParent) {
			this.keyboardParent.transform.local.scale =
				new MRE.Vector3(this.SpiralScale, this.SpiralScale, this.SpiralScale);
			this.updateKeyboardCenter();
		}
	}

	public updateKeyboardCenter() {
		//const highPos = this.computeKeyPositionX(this.keyHighest) * this.SpiralScale;

		//const offset = -highPos - 0.5;

		this.keyboardParent.transform.local.position.x = -1.0; //TODO base this on scale 
	}

	public setProperKeyColor(midiNote: number) {
		const note = midiNote % 12;

		let matt = this.ourApp.transparentBlackMat;

		if (this.zOffset[note] === 0) {
			matt = this.ourApp.transparentWhiteMat;
		}
		...
		*/
		...
		this.keyboardParent = MRE.Actor.Create(this.ourApp.context, {
			actor: {
				...
				transform: {
					local: {
						position: new MRE.Vector3(0, 0.5, 0),
						scale: new MRE.Vector3(this.SpiralScale, this.SpiralScale, this.SpiralScale)
					}
				}
			}
		});
		...
		this.ourApp.ourConsole.
			logMessage(`SPIRAL: creating new keyboard with range ${this.keyLowest} to ${this.keyHighest}`);
		//this.ourApp.ourConsole.logMessage(`octaves: ${totalOctaves}`);


		for (let i = this.keyLowest; i < this.keyHighest; i++) {
			const meshId: MRE.Guid = this.ourApp.sphereMesh.id;
			let mattId: MRE.Guid = this.ourApp.transparentBlackMat.id;
			const note = i % 12;
			const angleInRad=this.ourApp.degToRad(note/12*360);
			const radius=0.5;
			const xPos=Math.sin(angleInRad)*radius;
			const zPos=Math.cos(angleInRad)*radius;
			const yPos=((i-this.keyLowest)/12.0)*0.5;

			//const octave = Math.floor(i / 12);

			if (this.zOffset[note] === 0) {
				//meshId = whiteKeyMesh.id;
				mattId = this.ourApp.transparentWhiteMat.id;
			}

			const keyPos = new MRE.Vector3(xPos,yPos,zPos);
			...
			const keyActor = MRE.Actor.Create(this.ourApp.context, {
				actor: {
					...
					transform: {
						local: { 
							position: keyPos,
							scale: new MRE.Vector3(0.05,0.05,0.05)
						}
					},
					appearance:
					{
	...
	private setupInteractions(i: number) {
		const keyCollisionActor = this.ourKeys.get(i);

		keyCollisionActor.collider.onTrigger("trigger-enter", (otherActor: MRE.Actor) => {
			this.ourApp.ourConsole.logMessage("SPIRAL: trigger enter on SPIRAL note!");
		...
		buttonBehavior.onButton("pressed", (user: MRE.User, buttonData: MRE.ButtonEventData) => {
			if (this.isAuthorized(user)) {
				this.ourApp.ourConsole.logMessage("SPIRAL: user clicked on SPIRAL note: " + i);
				this.keyPressed(i, 100);
			} else {
				this.ourApp.ourConsole.logMessage("SPIRAL: user not authorized to click: " + i);
			}
		});
	...
	public keyPressed(note: number, vel: number) {
		this.ourApp.ourConsole.logMessage("SPIRAL received note ON message! note: " + note);
		...
		this.ourKeys.get(note).transform.local.scale=new MRE.Vector3(0.1,0.1,0.1);
	...
	public keyReleased(note: number) {
		//this.ourApp.ourConsole.logMessage("SPIRAL received note OFF message! note: " + note);

		if (!this.ourKeys.has(note)) {
			//this.ourApp.ourConsole.logMessage("ERROR: note is outside the range of our SPIRAL");
			return;
		}
		...
		this.ourKeys.get(note).transform.local.scale=new MRE.Vector3(0.05,0.05,0.05);
